...
citations verified against HEAD `911247d`._
...
- **Matcher** — `SetlistMatcher.ComputeProposals` / `MatchAndDecorate`: **name-gated, first-unclaimed**
  ...
  positions ([SetlistMatcher.cs:88-115](../Services/SetlistMatcher.cs)). No positional fallback; a
  track that matches no unclaimed entry is left untouched (never mis-named — follow-ups.md
  "setlist/media divergence is data-safe").
...
- **Manual / panel assignment** may claim an extra exactly as it claims a song
  (reference-side-panel-spec.md §7.5; the shared `ManualMatchApply.Apply` core already writes over
  `TrackInfo` + claimed set with no song/extra distinction).
...
**Sequence reference — reference-side-panel arc slice 5b.** Panel **click-to-assign** (and 6, DnD)
are specified in [reference-side-panel-spec.md](reference-side-panel-spec.md) §7.5/§11.5 and are
**NOT** part of this arc's slices. They are the placement gesture this arc's typed extras make useful:
5b should land **before** this arc's panel slice 3 so that, once extras are typed and displayed, the
user can actually assign a track to an extra entry. Recommended global order: side-panel 5b → extras
slice 1 → 2 → 3 → 4 (remap) → 5 (editor authoring, gated on 4) → 6 → 7.

## 11. Decision record (RESOLVED 2026-07-03)

All nine decisions were ratified 2026-07-03; the spec body above is written as settled design against
them. Each rejected alternative is retained here as a one-line "considered, rejected" note.
