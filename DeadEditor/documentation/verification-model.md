...
## Concert Surface (shipped 2026-06-10)

Verification extended to the concert store (`Data/concerts/*.json` via `ConcertLookupService`, the canonical setlist authority) as the third shipped surface. `ConcertReference.Verified` is a bare whole-record bool — no provenance, no per-track or per-set granularity — matching `AlbumManifest.Verified` and `BoxSetDefinition.Verified`; legacy fetcher-sourced files lacking the key deserialize to `false`, the correct default. The gate is a pure `ConcertVerifyGate` (date parses `yyyy-MM-dd`, venue non-empty, at least one track, every track having a song name), returning the first failing reason; notably it has **no** per-track-date rule (concert tracks inherit the concert date) and no city/state requirement. Unverify-on-edit uses **diff-at-save** like box sets: a normalized, serialized baseline (through the shared `ConcertSnapshot` projection, `LastUpdated` excluded) is snapshotted when `EditSetlistView` loads, and Save drops `Verified` when the current snapshot differs (`EditUnverifyRule.IsDirty`). The in-body verify surface in `EditSetlistView` carries an honest `effectiveVerified` badge (stored bool AND clean-vs-baseline), a Mark-Verified button (gated, in-memory, rebaselines), and a wrapping disabled-reason. **Concert-surface divergence:** unlike the album/box-set "no unlock gesture", concerts add an **explicit Unverify control** — trust can be withdrawn without a correction in hand (in-memory, persist-on-save, no rebaseline). Two further concert-only mechanisms: the `SetlistFetcher` **skip-verified guard** (re-fetches never clobber a curated, verified file) and a leading verified-glyph column in `ConcertDatabaseView` (✓ in the verified green, empty otherwise; binary — concerts are single-record, never `Partial`), reusing the Library-grid converters. Design authority: [concert-verification-spec.md](concert-verification-spec.md).

**Hand-created concerts (Add Concert, shipped 2026-06-10).** Concert records can now be created in-app, not only by the fetcher (see [add-concert-spec.md](add-concert-spec.md)). A hand-created concert is **born unverified** and earns verification through the same `ConcertVerifyGate`. Until it is verified, it is **not protected by the `SetlistFetcher` skip-verified guard** — for a date the fetcher's source knows about, the next full crawl will overwrite it. The guidance is therefore to verify hand-created concerts once correct; dates or artists outside the fetcher's source have no such exposure.
