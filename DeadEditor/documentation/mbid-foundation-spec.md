...
- ~~**Phase 2:** Import-time MBID write — persist `ReleaseOption.ReleaseId` from existing import flow into tags during import~~ — **Completed (Commit 1).** `AlbumInfo.MusicBrainzReleaseId` is set in [ImportView.ApplyMusicBrainzData](Views/ImportView.xaml.cs#L1309) from the user's `ReleaseSelectorDialog` choice and written to FLAC `MUSICBRAINZ_ALBUMID` and MP3 `MusicBrainz Album Id` TXXX in [LibraryImportService.WriteMetadataWithRetry](Services/LibraryImportService.cs#L228).
- ~~**Commit 1.6:** `MetadataService.WriteMetadata` parity — extend the in-place "Write Metadata" path (used by the ImportView button) to write MBID alongside the other 5 custom fields, mirroring Commit 1's pattern.~~ — **Completed (Commit 1.6).** [MetadataService.WriteMetadata](Services/MetadataService.cs#L332) now writes `MUSICBRAINZ_ALBUMID` (FLAC Xiph) and `MusicBrainz Album Id` (MP3 TXXX) when `AlbumInfo.MusicBrainzReleaseId` is non-empty, guarded so files without an MBID are untouched.
