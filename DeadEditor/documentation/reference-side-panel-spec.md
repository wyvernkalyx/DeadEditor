...
Before slice 5 the manual right-click Match-to-Song writes were **duplicated inline** in each code-behind with no shared entry point. That duplication is now gone: both handlers route through the shared apply core `ManualMatchApply.Apply` (`Helpers/ManualMatchApply.cs:30-48`; Import `ImportView.xaml.cs:936`, Edit `EditMetadataView.xaml.cs:1723`), per §7.1/§7.2 below. `MatchToSongDialog` still returns only the selected index and writes nothing.
...
- **Panel assignment gate = the panel is populated.** The panel's click/DnD assign is offered whenever the setlist projection exists (`_lastSetlistProjection != null` — Import `ImportView.xaml.cs:72,465,1366`; Edit `EditMetadataView.xaml.cs:65,1448,1840`). It is **NOT** gated on a completed Match Setlist run. Motivating case: **position-only imports where auto-match resolves nothing** — no run ever completes with claims, yet the setlist is populated on date entry (§5.2), so the panel is the *only* path to assign. A track's `IsMatched` state does not gate panel assignment (§7.5.1). **Interaction model (as built, slice 5b):** select a track in the grid, then **double-click** a setlist row; the panel raises `AssignRequested(position)` (single-clicks are ignored so an accidental click never assigns) and the host resolves the selected track — on Import (a full-row selection grid) directly from `TracksDataGrid.SelectedItem` (`ImportView.xaml.cs:1016`); on Edit (a `CellOrRowHeader` cell-selection grid, where a bare cell click leaves `SelectedItem` and `CurrentItem` null — the slice-5b gate-b bug) via `Helpers/SelectedTrackResolver`, which walks `SelectedItem` → **`CurrentCell.Item`** → `CurrentItem` (`EditMetadataView.xaml.cs:1789-1790`) — and applies the write. `SelectedTrackResolver` is therefore Edit-only; Import never calls it. The panel holds no selection and performs no write (§4).

- **Right-click menu gate = UNCHANGED.** The existing menu keeps its current, stricter gate: a completed **applied** Match Setlist run **and** the clicked track's `IsMatched != true` **and** at least one unclaimed setlist song. Ground truth: Import `ImportView.xaml.cs:844-852` (condition at `847-848`, unified per §7.6a: `_lastSetlistSongs != null && _lastClaimedPositions != null && _matchSetlistHasRun && vm.Track.IsMatched != true` — the run signal is now `_matchSetlistHasRun`, not the prior `_lastMatchDate != null`, plus the `BuildUnmatchedSetlistSongList().Count > 0` check at `851-852`); Edit `EditMetadataView.xaml.cs:1645-1650` (condition at `1646-1647`: `_matchSetlistHasRun && clickedTrack.IsMatched != true && _lastSetlistSongs != null && _lastClaimedPositions != null`, plus the unmatched-count check at `1649-1650`). Slice 5 does not change this gate; it only retrofits the handler body onto the shared core (§7.2).
...
**Kept-alive singleton is incidental, not load-bearing.** ImportView happens to be a kept-alive singleton (`ShellWindow.xaml.cs:29,553-572`), so in-progress import state survives the round-trip — but the design does not depend on it (the user re-Reads regardless).

**Edit-side deep-link (unbanked; direct-to-editor, guarded return).** Edit now exposes the same affordance (`ShowEditSetlistLink="True"`), wired differently from Import in two ways:
1. **Direct to the editor.** `EditMetadataView.OnEditSetlistRequested` resolves the concert by album date (same validation + null-guard as Import) and calls `ShellWindow.NavigateToSetlistEditor` (`new EditSetlistView` + `NavigateTo`), skipping Import's ConcertDetail hop. **Import's one-way path is deliberately unchanged.**
2. **Guarded refresh-on-return (Edit is NOT Read-driven).** The shell retains the `EditMetadataView` instance on the back stack and re-raises its `Loaded` on return (single `ContentPresenter.Content` swap, `ShellWindow.xaml:40-42`). A naive re-fire re-runs `LoadData`, which re-reads the FLAC tags and discards unsaved album edits + per-VM claims; a `_returningFromSetlistEdit` flag makes the return `Loaded` skip `LoadData` and instead **invalidate** match state (`_matchSetlistHasRun=false`; clear `_lastClaimedPositions`/`_lastSetlistSongs`/every `ClaimedSetlistPosition`; clear amber) and rebuild the panel as pure, undimmed reference. Claims are **invalidated, not remapped**: the editor can add/remove/renumber entries and shift the flattened positions every claim indexes, so until the position-preserving remap lands (the banked slice-4 obligation, setlist-extras-writeback-spec.md §3.3) a re-run of Match Setlist re-establishes claims against the new positions.

The earlier banking rationale ("`EditMetadataView` is reconstructed per entry, so a round-trip would lose edits") was imprecise — the instance is *reused* on the back stack; the real edit-loss vector is the `Loaded`/`LoadData` re-fire, which the guard closes.
...
DnD assignment (slice 6) is a thin surface over the slice-5 click-assign foundation: it reuses the same apply core (`ManualMatchApply.Reassign`), the same per-surface post-assign hooks, and the same per-row drop machinery the reorder feature already established. It adds only a drag source on the panel and a payload-typed drop branch. This section is corrected to the as-built 5b core — the pre-5b draft named `SetlistEntryVm` as the wire type and stated "net-new work is only the drag source"; both were wrong and are fixed below.

### 10.1 Payload — `SetlistReferencePanel.SetlistRow`, no dedicated wire type
- The drag carries the panel's own row VM, `SetlistReferencePanel.SetlistRow` (public, `SetlistReferencePanel.xaml.cs:139-148`), whose `Position` is the flattened 0-based claim axis. The host maps `Position` → `_lastSetlistProjection[position]` to recover `Canonical`/`Segue`, exactly as click-assign already does (`ImportView.xaml.cs:1031` / `EditMetadataView.xaml.cs:1805`).
- `SetlistEntryVm` is **NOT** the wire type. The panel projects `SetlistEntryVm` → `SetlistRow` inside `SetSetlist` and discards the projection (`SetlistReferencePanel.xaml.cs:104-136`), so a panel row does not hold a `SetlistEntryVm` to drag. The projection lives only in the host (`_lastSetlistProjection`), which is why the host — not the payload — carries `Canonical`/`Segue`.
- **Decision:** no dedicated drag-payload type. `SetlistRow` is already public and type-disjoint from `TrackInfoViewModel`, so it satisfies the mutual-exclusion contract (§10.2-ii) without a speculative second abstraction.

### 10.2 Net-new work (three parts)

**(i) Drag source on the panel row Border.** Capture the start point on `PreviewMouseLeftButtonDown` and call `DoDragDrop(border, row, Move)` only once the pointer crosses `SystemParameters.MinimumHorizontalDragDistance` / `MinimumVerticalDragDistance` on `PreviewMouseMove` — the proven reorder idiom (`ImportView.xaml.cs:1066-1082`). The drag-source handlers live in the panel code-behind (`SetlistReferencePanel.xaml.cs`) alongside the existing row template; initiating a drag is a display gesture, so the panel stays a pure display + event surface (§4) — the **host's** drop branch still owns the write. **Coexistence with the 5b double-click assign (as built).** "A stationary double-click never crosses the threshold" is necessary but **not sufficient**: `DoDragDrop` is a modal, blocking call, so if a drag starts at *any* point during the two-press double-click gesture it consumes the mouse-input stream and starves the bubbling `SetlistRow_MouseLeftButtonDown` of its `ClickCount == 2`, killing the assign (the gate-d failure). The canonical drag-source shape that makes the double-click win: the arm gate is `e.ClickCount == 1` on `PreviewMouseLeftButtonDown` — the double-click's second press never arms, so no drag can begin from it; armed state is cleared on `PreviewMouseLeftButtonUp`; `PreviewMouseMove` starts `DoDragDrop` only while the left button is held **and** movement exceeds `SystemParameters.Minimum{H,V}DragDistance`; and the `ClickCount == 2` branch disarms before raising `AssignRequested`. **Accepted residual:** a move past the threshold *during a single press* can still arm a drag — the identical tolerance the reorder grids already carry, and harmless in practice. **Manual-gate obligation:** verify on both surfaces that neither gesture eats the other — a double-click still assigns, and a deliberate drag still drags without firing an accidental assign.

**(ii) A `SetlistRow`-typed branch in `Row_DragOver`/`Row_Drop` on BOTH grids.** The existing handlers gate on `GetDataPresent(typeof(TrackInfoViewModel))` and reject every other payload with `e.Effects = DragDropEffects.None` (Import `ImportView.xaml.cs:1084-1101` DragOver / `1104-1125` Drop; Edit `EditMetadataView.xaml.cs:2011-2029` DragOver / `2031-2064` Drop), so a setlist drop is **silently refused** until a branch is added — this is not "only a drag source." Add an `else if (e.Data.GetDataPresent(typeof(SetlistReferencePanel.SetlistRow)))` branch to each of the four handlers. **Mutual-exclusion contract:** the two gates — `typeof(TrackInfoViewModel)` (reorder) vs `typeof(SetlistReferencePanel.SetlistRow)` (assign) — are type-disjoint and must stay so; a `GetDataPresent` mismatch is the documented **silent-fail** mode (edit-metadata-viewmodel-migration-inspection-2026-04-25.md).

**(iii) Per-surface `AssignEntryToTrack(TrackInfoViewModel vm, int position)` extraction.** The click path's body — refuse-steal + `ManualMatchApply.Reassign` + all inline post-assign hooks (panel re-dim, alias, status, `Items.Refresh`; Edit adds `ReconstructRawTitles`/`_hasUnsavedChanges`/`RefreshValidation`) — currently lives inside `SetlistPanel_AssignRequested(int)` fused to selection resolution (Import `ImportView.xaml.cs:1011-1048`; Edit `EditMetadataView.xaml.cs:1781-1826`). Extract that body into `AssignEntryToTrack(vm, position)` so both entry points share it: the click handler resolves the track from selection (§7.5) then calls it; the drop handler resolves the target from `targetRow.Item` (§10.3) then calls the same. No third copy of the write/hook set — the §7.2 "no third inline copy" rule extends to the drop path.

### 10.3 Target resolution — the drop row, never selection
DnD needs no grid selection and does not touch `SelectedTrackResolver` on either surface. The drop handler is attached per-row, so the target track is `targetRow.Item as TrackInfoViewModel` — the identical resolution the reorder Drop already uses (`ImportView.xaml.cs:1104-1108` / `EditMetadataView.xaml.cs:2031-2035`). `SelectedTrackResolver` exists only to disambiguate Edit's cell-selection grid for the *click* path (§7.5); DnD sidesteps that concern entirely because the row under the cursor is the target by construction.

### 10.4 Ordering
Ships **after** click-assign (slice 6, following slice 5), so assignment never depends on DnD mechanics; DnD is purely additive over the slice-5 `AssignEntryToTrack` seam.
...
6. **DnD assign** (formerly slice 5). Drag source on the panel setlist rows (threshold-armed, coexisting with the double-click assign gesture) + a `SetlistRow`-typed drop branch on BOTH grids' `Row_DragOver`/`Row_Drop`, resolving the target from the drop row (never selection) and routing through the shared `AssignEntryToTrack` seam extracted from click-assign (§10). WPF gate.
...
8. **No refresh-on-return hook (Import only)** — the user re-Reads by stated preference and Read already refreshes the panel; banked as the known small fix (§9, §13). *Edit-side is not Read-driven and therefore DOES refresh on return, via a guarded `Loaded` re-fire that invalidates claims and rebuilds the panel (§9).*
9. **Two-shows-one-date banked as a separate data-model arc** — the panel and round-trip work within the one-show model (§13).
10. **DnD payload is `SetlistReferencePanel.SetlistRow`, no dedicated wire type** — the row VM is already public and type-disjoint from `TrackInfoViewModel`, so it satisfies the reorder/assign mutual-exclusion gate without a speculative second payload type (§10.1).
11. **DnD drop-side target comes from `targetRow.Item`, never grid selection** — the drop row is the target, so DnD needs no `SelectedTrackResolver` on either surface (§10.3).
12. **`AssignEntryToTrack(vm, position)` is the shared click + drop seam per surface** — extracted from the click handler's body so the drop path reuses refuse-steal + `Reassign` + hooks with no third inline copy (§7.2, §10.2-iii).
...
- ~~**Edit-side deep-link affordance** (§9 asymmetry) — Import-first because `EditMetadataView` is reconstructed per entry, so an Edit-side round-trip would lose in-progress edits.~~ **IMPLEMENTED** (extras slice 6, pulled forward): direct-to-editor navigation + a guarded refresh-on-return that invalidates claims and skips the tag re-read (§9). The "reconstructed per entry" premise was imprecise — the instance is reused on the back stack; the guard closes the real `Loaded`/`LoadData` re-fire vector.
