...
### ~~Cold-load overlay for ConcertLookupService entry points~~ (Surfaced 2026-06-25; DONE 2026-06-26)
The first action after launch that touches `ConcertLookupService` stalls for several seconds with no feedback (per-process cache; colds once per launch). Edit Metadata's cold open was already fixed with `ConcertLookupService.IsLoaded` + `App.Alerts.RunWithStatusAsync`. Known unfixed entry points: (1) Import folder-Read (Read warms the concert DB for Match Setlist); (2) the Concerts nav button (loads the concert DB on first navigation). Likely others. Fix: rather than three one-off patches, audit all cold-`ConcertLookupService` entry points and wrap them with the existing overlay pattern — and consider a single shared guard at the `ConcertLookupService` access boundary instead of per-view. Own arc, separate from MB removal; wants its own read-only Phase A (enumerate every cold entry point + confirm `RunWithStatusAsync` reachability) and its own commit(s).

**Done 2026-06-26:** Shipped. Phase A (read-only) enumerated the cold-reachable entry points and found the shared-guard-at-the-service-boundary option a NO-GO — the boundary (`ShowLookupService.GetSetlist` / `ConcertLookupService.Instance`) is hit *synchronously*, and `App.Alerts.RunWithStatusAsync` is a UI concern that would pull a UI dependency into the service and break its layering/tests. Instead added `Helpers/ConcertDataGate.EnsureLoadedAsync` as the single shared idiom (short-circuits on `ConcertLookupService.IsLoaded`; otherwise forces the load off the UI thread under the status overlay, swallowing + bannering failures so callers degrade gracefully), and awaited it at the three active cold paths before their synchronous touch: **Concerts nav** (`ShellWindow.NavigateToConcerts`), **Import folder Read** (`ImportView.LoadFolderAsync`, before `RefreshUI`), and the **box-set wizard Pull-setlist** (`BoxSetWizardView.PullSetlistButton_Click`) — the last surfaced by Phase A beyond the two originally listed. Edit Metadata's inline guard was refactored onto the same helper (verbatim overlay strings + try/catch, behavior unchanged). Manual WPF gate PASS on every path; 412 tests pass, 50 unique warnings (unchanged).

- **Residual (low-urgency, NOT guarded by decision):** `ImportView.AlbumDateTextBox_LostFocus` -> `UpdateMatchSetlistButton` -> `ShowLookupService.GetSetlist` is cold-reachable on a fresh Import view before any Read (the Album Info bar is always editable). Phase A mislabeled this as `TextChanged`; it is **`LostFocus`**. Left unguarded because awaiting a `LostFocus` handler is the wrong trade. Adjacent observation: that handler does not null-check `_albumInfo`. Cross-ref the cold-load context above.

### Match Setlist: setlist/media divergence is data-safe; three follow-ups (Surfaced 2026-06-26)

Read-only diagnosis at HEAD a389fec confirmed Match Setlist is name-gated (`trackCanonical == entry.Canonical`, SetlistMatcher.cs:79) with no positional fallback. Under the real ways a taper's media diverges from the performance setlist — banter/tuning kept or stripped, songs combined or split, reprises — it degrades to "unmatched, untouched" and **can never write a wrong SongName**. A combined segue track (e.g. "Dark Star > St. Stephen > The Eleven") resolves to one unmatchable key and is left intact; it is *not* silently renamed to just "Dark Star" with the rest dropped. The data-safety question that prompted this is answered: no fix needed. Three items banked from it, all low-urgency:

- ~~**Duplicate matcher (maintenance hazard).**~~ (DONE 2026-06-26) Import uses the pure `SetlistMatcher.MatchAndDecorate`; `EditMetadataView` re-implements the same claim loop inline (EditMetadataView.xaml.cs:1661-1696). They agree today, but only the pure helper is tested — silent-divergence risk if one is changed without the other. Fix: converge Edit onto `MatchAndDecorate` (also the prerequisite step if a Match Setlist review surface is ever built). **Done 2026-06-26:** Edit Metadata's inline claim loop was replaced with a `SetlistMatcher.MatchAndDecorate` call using Import's resolver (`Normalize` -> `GetOfficialTitle`); both surfaces now run one tested matcher. A read-only audit confirmed the old inline loop and `MatchAndDecorate` have identical segue write-semantics, so convergence is behavior-preserving. Four SetlistMatcher tests added.
...
### Re-import creates a duplicate album folder when metadata drifts (Surfaced 2026-06-28)
Pre-existing, independent of the alias arc. Re-importing the same show can produce a second managed album folder and a merged 2×-track grid row (this is what the OFTV "32 tracks" gate observation was — NOT Fix A, whose file copy is idempotent on disk). Cause: the import duplicate check `ShowExistsInLibrary` (`LibraryImportService.cs:805-816`) matches the **exact** computed folder name (`BuildLibraryFolderName`), while the grid merges shows by **trimmed, case-insensitive ALBUMNAME** (`MergeOfficialReleasesByAlbumName`, `LibraryGridView.xaml.cs:1034-1070`). Any folder-name field drift between imports — an `(ASIN …)` album-name suffix, a venue/city spelling, or the date-present flag flipping the studio/live branch — yields a *different* folder, so the exact check misses it and a second folder is created and copied; the merge then aggregates both into one row with summed `TrackCount` and the same bare names twice. Fix direction: match on **show identity** (date + album) rather than the exact folder name, offering replace/update; scope alongside the folder-naming / disc-identity (Fix C) work.
